<template>
    <template v-if="isForSearch">
        <el-date-picker :placeholder="placeHolder==''?attribute.description:placeHolder" v-model="formModel.attr" type="datetime"  @change="valueChanged" :disabled="disabled"/>
    </template>
    <template v-else>
        <el-form :model="formModel" ref="refForm" :label-width="labelWidth + 'px'" display:flex flex-direction: row flex-wrap: wrap v-if="visible">
            <el-form-item :label="(lableText==''?attribute.displayName:lableText)" prop="attr" :title="(titleText==''?attribute.description:titleText)" :rules="rules()">
                <el-date-picker :placeholder="placeHolder==''?attribute.description:placeHolder" v-model="formModel.attr" type="datetime" :disabled="disabled||!attribute.isInput" @change="valueChanged" />
            </el-form-item>
        </el-form>
    </template>
</template>
