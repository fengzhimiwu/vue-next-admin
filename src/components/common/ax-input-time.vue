<template>
    <template v-if="isForSearch">
        <el-time-picker  :placeholder="placeHolder==''?attribute.description:placeHolder" v-model="formModel.attr"  format="HH:mm" @change="valueChanged" :disabled="disabled"/>
    </template>
    <template v-else>
        <el-form :model="formModel" ref="refForm" :label-width="labelWidth + 'px'" display:flex flex-direction: row flex-wrap: wrap v-if="visible">
            <el-form-item :label="(lableText==''?attribute.displayName:lableText)" prop="attr" :title="(titleText==''?attribute.description:titleText)" :rules="rules()">
                <el-time-picker  :placeholder="placeHolder==''?attribute.description:placeHolder" v-model="formModel.attr" :disabled="disabled||!attribute.isInput" format="HH:mm" @change="valueChanged" />
            </el-form-item>
        </el-form>
    </template>
</template>
